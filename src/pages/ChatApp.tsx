    ...
    <div className="mx-auto max-w-6xl px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-2 space-y-4">
        <div className="card p-4 h-[70vh] overflow-y-auto space-y-4">
          {messages.length === 0 && (
            <div className="text-neutral-400">Ask anything about 21 CFR Part 11, ICH E6(R3), CSV, privacy, and more.</div>
          )}
          {messages.map((m, i) => (
        ...
        <div className="flex items-start gap-3">
          <textarea
            ...
            className="w-full min-h-[64px] p-3"
          />
      ...
      <aside className="md:col-span-1">
        <div className="card p-4 sticky top-20">
          ...
          <div className="space-y-3">
            {resources.length === 0 ? (
              <p className="text-neutral-400">Resources from each answer will appear here.</p>
            ) : (
              resources.map((r, i) => (
                <div key={i}>
                  <a href={r.url} target="_blank" rel="noreferrer">{r.title}</a>
                  {r.source && <div className="text-xs text-neutral-400">{r.source}</div>}
                </div>
              ))
            )}
          </div>
          {settings && (
            <div className="text-xs text-neutral-500 mt-4">
              Domains allowed: {(settings.allowedDomains || []).join(", ") || "any"}
            </div>
          )}
        </div>
      </aside>
