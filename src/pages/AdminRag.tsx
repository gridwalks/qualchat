...
export default function AdminRag() {
  ...
  async function onSave(e: React.FormEvent) {
    e.preventDefault();
    const res = await ragUpsert(form);
    setMsg(`Saved ${res.id}`);
    setForm({ title: "", url: "", content: "", keywords: [] });
    await refresh();
  }
      ...
      <form onSubmit={onSave} className="card p-4 space-y-3 mb-6">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full p-2" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required />
          </div>
          <div>
            <label className="block text-sm mb-1">URL</label>
            <input className="w-full p-2" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} required />
          </div>
        </div>
        <div>
          <label className="block text-sm mb-1">Keywords (comma-separated)</label>
          <input className="w-full p-2" value={(form.keywords || []).join(", ")}
            onChange={(e) => setForm({ ...form, keywords: e.target.value.split(",").map(s => s.trim()).filter(Boolean) })} />
        </div>
        <div>
          <label className="block text-sm mb-1">Content (used for simple keyword match)</label>
          <textarea className="w-full h-40 p-3" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} />
        </div>
