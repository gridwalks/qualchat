...
export default function AdminLogs() {
  ...
  React.useEffect(() => {
    logsAdmin().then(setLogs).catch((e) => setErr(String(e))).finally(() => setLoading(false));
  }, []);

  async function onExport() {
    const blob = await exportLogsAdmin();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "acceleraqa-logs-admin.csv";
    a.click();
        ...
        <div className="space-y-3">
          {logs.map((l) => (
            <div key={l.id} className="card p-4">
              ...
              <div className="mt-2 font-semibold">Q:</div>
              <div className="whitespace-pre-wrap">{l.question}</div>
              <div className="mt-2 font-semibold">A:</div>
              <div className="whitespace-pre-wrap">{l.answer}</div>
            </div>
          ))}
        </div>
      )}
