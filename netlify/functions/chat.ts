...
function extractResources(answer: string) {
  ...
  try {
    if (fence && fence[1]) {
      ...
      if (Array.isArray(parsed.resources)) {
        return parsed.resources.filter((r: any) => r && r.title && r.url).map((r: any) => ({
  ...
  // 2) fallback: markdown links [title](url)
  const links: any[] = [];
  const re = /\[([^\]]+)\]\((https?:[^\)\s]+)\)/g;
  let m;
...
function filterByDomains(resources: any[], allowed: string[]) {
  if (!allowed || allowed.length === 0) return resources;
  const domains = allowed.map((d) => d.toLowerCase());
  return resources.filter((r) => {
    try {
      const u = new URL(r.url);
      return domains.some((d) => u.hostname.toLowerCase().endswith(d) || u.hostname.toLowerCase() === d);
    } catch { return false; }
  });
}
...
async function ragRetrieve(message: string, k: number) {
  if (k <= 0) return [];
  const listed = await ragStore.list({ prefix: "doc:" });
  const docs: any[] = [];
  for (const b of listed.blobs) {
    const d = await ragStore.getJSON(b.key);
    if (d) docs.push(d);
  }
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    const ragText = ragDocs.map((d) => `- ${d.title}: ${d.content}` ).join("\n");

    const sys = settings.sys_prompt;
    const userMsg = `User question:\n${message}\n\nIf helpful, you may suggest resources. Output answer first, then include a single fenced JSON block with \"resources\" array (title, url, source).`;
    ...
    try {
      ...
      const resp = await client.chat.completions.create({
        model,
        messages: [
          { role: "system", content: sys + ragMsg },
          { role: "user", content: userMsg },
        ],
        temperature: 0.2,
      });
      answer = resp.choices?.[0]?.message?.content || "";
      if (!answer.trim()) throw new Error("Empty chat completion");
    } catch (e) {
      api = "responses";
    ...
    let resources = extractResources(answer);
    resources = filterByDomains(resources, settings.allowedDomains || []);
    if ((settings.resourcesMax ?? 6) > 0) {
      resources = resources.slice(0, settings.resourcesMax);
    }
    ...
    await logUserEntry(user, entry);
    await logAdminEntry({ ...entry, ragDocs });
